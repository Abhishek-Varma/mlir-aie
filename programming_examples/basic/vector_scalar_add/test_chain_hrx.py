...
The HRX analogue of the XRT ``test_runlist.cpp`` testbench: it runs two
dispatches of the same ``x + 1`` kernel as a single batched submit
(``HRXHostRuntime.run_chain``), where the second run consumes the first run's
output:
...
def main():
    ...
    for k, st in enumerate(stages):
        ...
        if not np.array_equal(got, ref):
            ...
            print(f"deep-chain stage {k} mismatch @ {bad}: got {got[bad]} != {ref[bad]}")
            errors += int(np.count_nonzero(got != ref))
    if errors:
        print(f"\nfailed ({errors} mismatches in deep chain).\n")
        return 1
    print(f"deep chain (depth {depth}): stage k == in + (k+1)  "
