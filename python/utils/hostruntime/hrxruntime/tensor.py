...
This mirrors XRTTensor but swaps the pyxrt ``xrt.bo`` for an HRX persistent
buffer (the ``BoImpl`` strategy from ``xrt_coreutil_shim.cpp``): the engine
reads/writes the mapping directly, so there is no host staging copy. Coherence
around device work is maintained with cheap cache ops:
